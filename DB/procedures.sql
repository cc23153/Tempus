...
create or alter procedure [Tempus].[spUpdateTaskCategory]
    @task_id int,
...
begin
    ...
    if not exists (select 1 from [Tempus].[Category] where category_id = @task_new_category)
    begin 
        raiserror('The Category doesn''t exist', 16, 1)
        return
    end
    begin try 
        begin transaction
        update [Tempus].[Task] set task_category = @task_new_category where task_id = @task_id
        commit
    end try 
